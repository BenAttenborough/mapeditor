...
function UI:render()
    love.graphics.draw(self.tileGrass, 20, 20)
    love.graphics.draw(self.tileDirt, 20, 40)
    love.graphics.draw(self.tileWater, 20, 60)
    -- print(selectedTile)
end
...
function UI:detectClick(x,y)
    if self:isClickWithinTile(20, 20, push:toGame(x,y)) then
        print("Grass selected")
        selectedTile = 1
    end
    if self:isClickWithinTile(20, 40, push:toGame(x,y)) then
        print("Dirt selected")
        selectedTile = 2
    end
    if self:isClickWithinTile(20, 60, push:toGame(x,y)) then
